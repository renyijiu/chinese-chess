...
async function boardScreenPoint(
  ...
  const verticalWorldSpan =
    2 * Math.tan((CAMERA_FOV_DEGREES * Math.PI) / 360) * (cameraHeight - worldY);
...
export async function waitForEnvironmentSettled(page: Page, expected?: "ready" | "degraded") {
  const viewer = page.locator(".board-viewer");
  await expect(viewer).toHaveAttribute("data-environment-status", expected ?? /^(ready|degraded)$/);
}
...
export async function openCleanGame(
  ...
  await page.addInitScript(
    ({ initialQuality, initialReducedMotion }) => {
  ...
  await expect(page.locator(".xiangqi-game-shell")).toHaveAttribute(
    "data-reduced-motion",
